...
#' try(
#'   vicmap_get_data(record = '76b1b7a3-2112-4444-857a-afccf7b20da8',
#'                 resource = '4d0377d9-e8a1-429b-824f-0ce8f363512c')
#' )
#'
#' try(
#'   vicmap_get_data('1d21922b-ec4f-42e5-8f6b-bf320a286157')
#' )
#'
...
#'
#' try(
#'   vicmap_get_data(record)
#' )
#'
#' # Using a BCGW name
#' try(
#'   vicmap_get_data("WHSE_IMAGERY_AND_BASE_MAPS.GSR_AIRPORTS_SVW")
#' )
#'
...
#' try(
#'   vicmap_get_data('d7e6c8c7-052f-4f06-b178-74c02c243ea4')
#' )
#'
...
#' ## multiple worksheets
#' try(
#'  vicmap_get_data('8620ce82-4943-43c4-9932-40730a0255d6')
#' )
#'
#' ## we can control what is read in from an excel file
#' ## using arguments from readxl::read_excel
#' try(
#'   vicmap_get_data('8620ce82-4943-43c4-9932-40730a0255d6', sheet = 'Regional Districts')
#' )
#' }
#'
#' @export
vicmap_get_data <- function(record, resource = NULL, verbose = TRUE, ...) {
  if (!has_internet()) stop("No access to internet", call. = FALSE) # nocov
  UseMethod("vicmap_get_data")
}

#' @export
vicmap_get_data.default <- function(record, resource = NULL, verbose = TRUE, ...) {
  stop("No vicmap_get_data method for an object of class ", class(record),
       call. = FALSE)
}

#' @export
vicmap_get_data.character <- function(record, resource = NULL, verbose = TRUE, ...) {
  ...
  vicmap_get_data(x, resource, verbose = verbose, ...)
}

#' @export
vicmap_get_data.bcdc_record <- function(record, resource = NULL, verbose = TRUE, ...) {
  record_id <- record$id
  ...
  if (name_choice == "WFS request (Spatial Data)") {
    ...
    # cat_line_wrap(glue::glue("vicmap_get_data('{x}', resource = '{id_choice}')"))
    query <- bcdc_query_geodata(record = record_id, ...)
    return(collect(query))
  } else {
    ...
    message("To directly access this record in the future please use this command:\n",
            glue::glue("vicmap_get_data('{record_id}', resource = '{id_choice}')"),"\n")
...
#' can be passed through the `vicmap_get_data` function to the reading function. This is
